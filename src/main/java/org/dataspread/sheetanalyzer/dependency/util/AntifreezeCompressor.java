...
public class AntifreezeCompressor {

    public static Map<Ref, Set<Ref>> buildCompressedGraph(List<Pair<Ref, Ref>> edgeBatch,
        ...
        Map<Ref, Set<Ref>> compressedGraph = new HashMap<>();
        for (Ref precCell : uniquePrecCells) {
            List<Ref> list = new LinkedList<>(getDeps(singleDepMap, depGraph, precCell));
            List<Ref> compList = compress(list, compressionConstant);
            compressedGraph.put(precCell, new HashSet<>(compList));
        }
